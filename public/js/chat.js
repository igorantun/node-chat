...
var connect = function () {
  ...
  socket.onmessage = function (e) {
    ...
    if (data.type === 'typing') {
      ...
      document.getElementById('typing').innerHTML = string
      return
    }
    ...
    if (data.type === 'server') {
      ...
      switch (data.info) {
        ...
        case 'connection':
          userip = data.user.ip ? ' [' + data.user.ip + ']' : ''
...
$(document).ready(function () {
  ...
  $('#recognition').bind('change', function () {
    ...
    if (settings.recognition) { $('#audio').show() } else {
      $('#audio').hide()
    }
  })
...
if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
  ...
  var speechToText = new webkitSpeechRecognition() // eslint-disable-line new-cap
